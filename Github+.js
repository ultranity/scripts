// ==UserScript==
// @name         Github+
// @namespace    https://github.com/ultranity/scripts/Github+.js
//@homepage https://github.com/ultranity/scripts
// @version      1.0.0
// @description  添加Github文件raw下载、复制按钮、图片点击放大(右击恢复)、nbviewer打开ipynb文件、cdn镜像加速等功能，适配 Refined GitHub
...
// @require      https://greasyfork.org/scripts/53536-ui/code/UI.js?version=281393 //showTips
// @author       ultranity
...
(function() {
    ...
    function createDownLink(){
        ...
        path = path.replace('tree','raw');
            var refined = false;
        if($files[0].parentNode.className == "rgh-edit-files-faster")
            refined = true;
        $files.each(function(i,fileElm){
            if(refined){
                var trElm = fileElm.parentNode.parentNode.parentNode,
                    cntElm = fileElm.parentNode.parentNode.nextElementSibling.querySelector('span');
            }else{
                trElm = fileElm.parentNode.parentNode,
                cntElm = fileElm.parentNode.nextElementSibling.querySelector('span');
            }
            var    cntA = cntElm.querySelector('a'),
        ...
        $directory.each(function(i,dirElm){
            var trElm = dirElm.parentNode.parentNode,
                cntElm = dirElm.parentNode.nextElementSibling.querySelector('span'),
                cntCssTruncateA = cntElm.querySelector('.js-navigation-open'),
    ...
    function openNbviewer(){
        ...
        console.log('There');
        var title = $('.final-path')[0].parentNode,
              a = $('<a></a>');
        a.text('===>\t\topen in nbviewer');
        a.attr({class:'openNbviewer','href':"https://nbviewer.jupyter.org/github/"+addr});
        //a.css({cursor:'pointer',visibility:'hidden'});
        title.appendChild(a.get(0));
        a.on('click',linkClick);
        window.open();
    }
    /**
